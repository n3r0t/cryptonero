...
class CryptoCommands(commands.Cog):
    ...
    async def comm_get_trending(self, ctx):
        ...
        try:
            ...
            for coin in trending['coins']:
                ...
                embed.add_field(name=f"24h change", value=f"{round(top7_dict[coin['item']['id']]['usd_24h_change'], 2)}%", inline=True)
    ...
    async def comm_getcurr(self, ctx, arg):
        ...
        try:
            ...
            circulating_supply = await self.localizeNB(coin['market_data']['circulating_supply'])
            if coin['market_data']['total_supply']:
